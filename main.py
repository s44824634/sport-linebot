...
def run_crawler(target, during_key, num):
    ...
    lines = [f"🏆 {target} 主推 Top10（{during_key}・{collected}人）\n"]
...
def handle_message(event):
    ...
    with ApiClient(configuration) as api_client:
        ...
        if text in ["help", "Help", "說明", "使用說明"]:
            ...
            try:
                num = int(parts[2])
                reply = f"⏳ 開始爬取 {parts[0]} {parts[1]} {num}人資料，約需 3~8 分鐘..."
                ...
                line_bot_api.push_message(
                    push_message_request={"to": event.source.user_id, "messages": [{"type": "text", "text": result}]}
                )
                return
            except Exception as e:
                reply = f"❌ 發生錯誤: {str(e)}"
        else:
            reply = "❓ 格式錯誤，請輸入「說明」查看使用方式"
